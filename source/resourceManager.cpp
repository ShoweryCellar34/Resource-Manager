...
namespace RM {
    ...
    mode::mode() : m_read(false), m_write(false) {
    }

    mode::mode(bool read, bool write) : m_read(read), m_write(write) {
    }

    std::ios_base::openmode mode::toOpenmode() const {
    ...
    resource::resource(const fs::path& path, mode permitions, bool mustExist) {
    ...
    mode resource::getPermitions() const {
        return m_permitions;
    }
    ...
    resource* resourceManager::loadResource(const std::string& alias, const fs::path& path, mode permitions, bool mustExist) {
